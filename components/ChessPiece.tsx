...
export function ChessPiece({ type, color, square, disabled = false }: ChessPieceProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`w-full h-full flex items-center justify-center ${disabled ? "" : "cursor-grab active:cursor-grabbing"}`}
      role="img"
      aria-label={`${color === "w" ? "White" : "Black"} ${pieceNames[type] || type}`}
    >
      {renderPiece()}
    </div>
  );
